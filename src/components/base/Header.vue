<template>
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    <router-link class="navbar-brand" to="/">Engagement Platform</router-link>
...
<script>
import logoutService from '@/services/auth_service/LoginService'

export default {
  methods: {
    logout () {
      logoutService.logOut().then(this.$store.dispatch('user/logout')).catch(error => console.log(error))
    }
  },
  computed: {
    isAuthenticated () {
